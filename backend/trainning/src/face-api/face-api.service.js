...
class FaceApiService {
  ...
  async loadTrainingData() {
    ...
    const dataTrainingsFilter = dataTrainings.filter(
      (dataTraining) => dataTraining.image.length > 0
    );
    for (const dataTraining of dataTrainingsFilter) {
      ...
      faceDescriptors.push(
        new faceapi.LabeledFaceDescriptors(dataTraining.name, descriptions)
      );
    }
  ...
  async search(file) {
    const img = await faceapi.bufferToImage(Buffer.from(file.buffer));
    const detections = await faceapi
      .detectAllFaces(img)
      .withFaceLandmarks()
      .withFaceDescriptors();
